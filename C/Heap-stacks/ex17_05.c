...
void Database_findbyProf(struct Connection *conn, char *name)
{
    int i = 0;
    for (i = 0; i < conn->db->max_rows; i++) {
        struct Address *addr = &conn->db->rows[i];
...
void Database_findbyNumb(struct Connection *conn, char *numb)
{
    int i = 0;
    for (i = 0; i < conn->db->max_rows; i++) {
        struct Address *addr = &conn->db->rows[i];
...
int main(int argc, char *argv[])
{
    ...
    switch(action) {
        ...
        case 'p':
            if (argc != 4) die("Need a search token", conn);
            Database_findbyProf(conn, argv[3]);
            break;
        case 'n':
            if (argc != 4) die("Need a search token", conn);
            Database_findbyNumb(conn, argv[3]);
            break;
