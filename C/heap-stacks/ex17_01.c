/*
 *
 * The die function needs to be augmented to let you pass the conn variable 
...
// UPDATE #1: so that die function can use the 
// Database_close function before it's defined
void Database_close(struct Connection *conn);

// a function to kill the program with an error.
// used after any failed function of bad input to exit an error
// using exit
// UPDATE 01: now does the cleanup from Database_close function
...
int main(int argc, char *argv[])
{
    ...
    switch(action) {
        ...
        // if action is 'd' deletes a row with given id and then writes to database
        case 'd':
